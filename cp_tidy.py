...
class TidyDataFrame:
    data: pyspark.sql.DataFrame = field(
        ...
        repr=lambda self: f"DataFrame [# rows x # cols]",
    )
    ...
    toggle_optimize: bool = field(default=False, validator=validators.instance_of(bool))
    n_rows: int = field(default=None)  # should be private!
    ...
    def _log_operation(
        self, operation="custom", message="method not covered by TidyDataFrame"
    ):
        """Simple logger invoked by masked methods."""
        # TODO: consider loguru?
    ...
    def display(self):
        ...
        if not self.toggle_display:
            self._log_operation(operation="display", message="feature toggled off")
        else:
            self.data.display()
    ...
    def select(self, *cols):
        ...
        def get_message(pre: list[str], post: list[str]) -> str:
            n_pre = len(pre)
            n_post = len(post)
            assert n_post <= n_pre, "Unsure how select returned more columns?"
            if n_pre == n_post:
